...
def check_if_workday_started(user_id: int, db: SessionLocal):
    return (
        ...
        .filter(
            ...
            TimeLog.date == datetime.now().date(),
            TimeLog.short_desc == "SYSTEM",
            TimeLog.comment == "Workday started",
        )
        .first()
    )
...
def check_if_workday_paused(user_id: int, db: SessionLocal):
    return (
        ...
        .filter(
            ...
            TimeLog.date == datetime.now().date(),
            TimeLog.short_desc == "SYSTEM",
            TimeLog.comment == "Workday paused",
        )
        .first()
    )
...
def start_workday(
    ...
    if (
        db.query(TimeLog)
        .filter(TimeLog.user_id == user_id, TimeLog.date == datetime.now().date())
        .first()
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Workday already started"
        )
...
def pause_worklog(
    ...
    if not check_if_workday_started(user.id, db):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Workday not started"
        )
    if check_if_workday_paused(user.id, db):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Workday already paused"
        )
...
def resume_worklog(
    ...
    if not check_if_workday_paused(user.id, db):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Workday not paused"
        )
...
def end_workday(
    ...
    if check_if_workday_paused(user.id, db):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Workday paused"
        )
...
def get_timelog_by_date(
    date: str, token: str = Depends(oauth2_scheme), db: SessionLocal = Depends(get_db)
):
    verify_token(token)
    ...
    timelogs = (
        db.query(TimeLog)
        .filter(TimeLog.user_id == user.id, TimeLog.date == convert_to_date(date))
        .all()
    )
    return timelogs
...
def get_worked_time_csv(
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=worked_time.csv"},
    )
